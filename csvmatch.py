...
def run(
    ...
    if weightings is None:
        weightings = [1 / len(fields1) for _ in range(len(fields1))]
    if len(weightings) != len(fields1):
        raise Exception(
            "list of weightings must be as long as the number of columns specified"
        )
    if sum(weightings) != 1.0:
        raise Exception("sum of weightings must equal to 1.0")
    ...
    tick = (
        ticker("Matching", len(processed1) * len(processed2))
        if ticker and algorithm is not "bilenko"
        else None
    )
    if algorithm is None:
        matches = matcher(algorithm)(
            processed1, processed2, fields1, fields2, threshold, tick
        )
    else:
        matches = matcher(algorithm)(
            processed1, processed2, fields1, fields2, threshold, tick, weightings
        )
...
def process_filter(filters, ignore_case):
    ...
    def filterer(data):
        regex = re.compile(
            "(" + "|".join(filters) + ")", re.IGNORECASE if ignore_case else 0
        )
...
def process_filter_titles(ignore_case):
    filename = os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "csvmatch-titles.txt.py"
    )
...
def matcher(algorithm):
    if algorithm == None:
        ...
        raise Exception(algorithm + ": algorithm does not exist")
...
def format(output, headers1, headers2, fields1, fields2):
    ...
    for definition in output:
        if re.match("^[1|2]\..*", definition):  # standard header definitions
            ...
            if (header[0] == "1" and header[1] not in headers1) or (
                header[0] == "2" and header[1] not in headers2
            ):
                raise Exception(header[1] + ": field not found in file " + header[0])
            outputs.append((header[0], header[1]))
        elif re.match(
            "^[1|2]\*$", definition
        ):  # expand 1* and 2* to all columns from that file
            number = definition.split("*")[0]
            if number == "1":
                outputs = outputs + [("1", key) for key in headers1]
            elif number == "2":
                outputs = outputs + [("2", key) for key in headers2]
        elif definition == "degree":  # the matching degree
            outputs.append(("-", "degree"))
        else:
            raise Exception(
                "output format must be the file number, followed by a dot, followed by the name of the column"
            )
    return outputs
