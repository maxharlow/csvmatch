...
):  # threshold and weighting not used, since automatically calculated
    input1 = {
        i: {fields1[j]: value for j, value in enumerate(row)}
    ...
    while not finished:
        for pair in linker.uncertainPairs():
            ...
            for record in pair:
                sys.stderr.write("\n")
                for field in set(
                    field.field for field in linker.data_model.primary_fields
                ):
                    sys.stderr.write(
                        colorama.Style.BRIGHT
                        + field
